...
(async () => {
    try {
        ...
        await page.type('.search-global-typeahead__input', process.env.USERKEYWORD);
        ...
        await page.type(inputFieldSelector, process.env.USERLOCATION);
        await page.waitForTimeout(1000);
        ...
        await page.type('.search-reusables__filter-value-item:nth-child(3) input[type="text"]', process.env.USERJOBTITLE);
        ...
        for (let currentPage = 1; currentPage <= process.env.MAX_PAGES; currentPage++) {
            // Build the URL with the extracted geoUrn for subsequent pages
            const url = `https://www.linkedin.com/search/results/people/?geoUrn=${geoUrn}&keywords=${process.env.USERKEYWORD}&origin=FACETED_SEARCH&page=${currentPage}&titleFreeText=${process.env.USERJOBTITLE}`;
        ...
        const csvWriter = createCsvWriter({
            path: process.env.CSV_FILE_PATH || 'linkedin_profiles.csv', // Use config.csvFilePath if available, otherwise default to 'linkedin_profiles.csv'
            header: [
                { id: 'name', title: 'Name' },
